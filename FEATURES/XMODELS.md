...
The Dynamic Model Registry provides runtime model registration and management with automatic UI widget detection, permissions, and audit logging. It works in conjunction with the Inspector module, which handles schema discovery and model generation.
...
class DynamicModelRegistry:
    ...
    def register_model(
        ...
        self.ui_schemas[name] = UISchemaGenerator.detect_widgets(model_class)
...
class UISchemaGenerator:
    """Detect UI widgets from Pydantic models"""

    @staticmethod
    def detect_widgets(model_class: Type[BaseModel]) -> UISchema:
        """Detect appropriate UI widgets from model fields"""
        fields = {}
...
class DynamicModelRegistry:
    ...
    def register_from_inspector(
        self,
        model_class: Type[BaseModel],
        model_name: str = None
    ) -> str:
        """Register a model that was already generated by XInspector"""
        
        # Models registry only handles registration, not generation
        # Model must be pre-generated by XInspector via two-step process:
        # 1. Inspector generates model
        # 2. Models registry registers it
...
### Integration with XInspector (Two-Step Process)

```python
from src.server.core.inspector import XInspector
from src.server.core.resources import XResource

# Step 1: Inspector generates model from data source
mongo_resource = MongoDBResource(connection_string="mongodb://localhost:27017/app")
await mongo_resource.connect()

inspector = XInspector(mongo_resource)
UserModel = await inspector.generate_model("users")  # Inspector analyzes & generates

# Step 2: Models registry registers the generated model
registry = DynamicModelRegistry()
model_name = registry.register_from_inspector(UserModel, "User")

# Now the model is available for runtime operations
user_data = registry.create("User", {
    "name": "John Doe", 
...
def test_ui_widget_detection():
    ...
    schema = UISchemaGenerator.detect_widgets(UserModel)
...
The Models registry integrates with other components following strict separation of concerns:

### With XInspector (Model Generation)
1. **XInspector analyzes** data sources and generates Pydantic models
2. **XModels registers** the generated models for runtime use
3. Clear two-step process prevents coupling

### With XRepo (Data Access)  
- Repos use registered models for type-safe operations
- Models provide metadata for query optimization
- Registry handles model lifecycle, Repo handles data access

### With XResource (Data Sources)
- Resources provide raw data connections
- Inspector analyzes Resources to generate models  
- Models registry stores the generated models for reuse

This separation ensures each component has a single responsibility:
- **XInspector**: Schema discovery and model generation
- **XModels**: Model registration and runtime management  
- **XRepo**: Data access operations
- **XResource**: Connection management
